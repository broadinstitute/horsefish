#!/usr/bin/env python
# python3 streaming_upload/incremental_upload.py -a '' -p 'fc-156ce5ce-ac56-40ee-adcc-71ad6dc9d351' -r '/Users/msimeon/horsefish/streaming_upload/miseq/201112_M04004_0509_000000000-DB53C' -t '/Users/msimeon/horsefish/streaming_upload/upload/TMP' -L '/Users/msimeon/horsefish/streaming_upload/upload/LOGS' -m 1000 -z 500 -i 900 -R 3 -D '24h' -I 2 -u 8 --verbose -w 'merge_tar_chunks' --downstream-input ''
...
def parse_args():
    ...
    parser.add_argument("-a", "--api-token", metavar="<token>", required=True,
            help="API token to authenticate against Terra platform.")
    ...
    downstream_analysis_group.add_argument("-A", "--applet", metavar="<applet-id>",
            help="applet id to execute after the RUN folder has been " +
    ...
    downstream_analysis_group.add_argument("-w", "--workflow", metavar="<workflow-id>",
            help="workflow id to execute after the RUN fodler has been " +
...
def check_input(args):
    ...
    if args.workflow:
        try:
            fapi.get_repository_method("broad-firecloud-dsde", "merge_tar_chunks", "master", wdl_only=False)
        except:
            raise_error("Unable to resolve workflow %s. %s" %(args.workflow))
    ...
    try:
        ...
        sub.check_call(['python3', '{curr_dir}/sync_directory.py'.format(curr_dir=sys.path[0]), '-h'],
                stdout=open(os.devnull, 'w'), close_fds=True)
    except sub.CalledProcessError:
        raise_error("sync_directory.py not found. Please run incremental " +
                "upload from the directory containing incremental_upload.py "+
                "and sync_directory.py")
...
def run_sync_dir(lane, args, finish=False):
    ...
    invocation = ["python3", "{curr_dir}/sync_directory.py".format(curr_dir=sys.path[0])]
    invocation.extend(["--log-file", lane["log_path"]])
    invocation.extend(["--project", args.project])
    invocation.extend(["--tar-destination", lane["remote_folder"]])
