#!/usr/bin/env python
#python3 streaming_upload/monitor_runs.py -c streaming_upload/monitor_runs.config -p "fc-156ce5ce-ac56-40ee-adcc-71ad6dc9d351" -d streaming_upload/miseq -w "merge_tar_chunks" -v
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Script to monitor a local directory for new sequencing RUNS and\n' +
                                                  'trigger the incremental upload script when a new RUN directory not net synced\n' +
                                                  'to the Terra platform is observed.\n' +
    ...
    downstreamAnalysis.add_argument('--applet', '-A',
                        help='ID of app(let) to execute after successful ' +
    ...
    downstreamAnalysis.add_argument('--workflow', '-w',
                        help='ID of workflow to execute after successful ' +
                             'upload of the RUN folder, see incremental_upload.py ' +
...
# TODO: Check if need yalm or workflow
def get_streaming_config(config_file, project, applet, workflow, script, token):
    ...
    if script:
        ...
        #print("script")
    #TODO Remove config
    user_config_dict = {"log_dir": '~/horsefish/streaming_upload/upload/LOGS', 
                        "tmp_dir": '~/horsefish/streaming_upload/upload/TMP', 
                        "exclude": '', 
                        "n_retries": 3, 
                        "run_length": "24h", 
                        "n_seq_intervals": 2,
...
# TODO change to check google bucket instead and run object instead of folders
def check_dnax_folders(run_folders, project):
    ...
    try:
        ...
        sys.exit("Invalid project ID ({0}) given. {1}".format(project))

    try:
        print(run_folders)
        blob = bucket.blob("201112_M04004_0509_000000000-DB53C/RunInfo.xml")
        print(blob)
        res = blob.upload_from_filename("streaming_upload/miseq/201112_M04004_0509_000000000-DB53C/RunInfo.xml")
        print(res)
        print(
        "File {} uploaded to {}.".format(
            "201112_M04004_0509_000000000-DB53C/RunInfo.xml", "201112_M04004_0509_000000000-DB53C/RunInfo.xml"
        )
        )
        ...
        # Go through object not folders
        folders = []
        blobs = storage_client.list_blobs(project)
        for blob_object in blobs:
            folders.append(blob_object.name)
            print(blob_object.name)
        folders = [os.path.basename(folder) for folder in folders]

        synced_folders = filter( (lambda folder: folder in folders), run_folders)
        unsynced_folders = filter( (lambda folder: folder not in folders), run_folders)
...
# TODO see if this is like Terra project
...
def main():
    ...
    if (not os.path.isfile("{0}/{1}".format(curr_dir, 'incremental_upload.py')) or
        not os.path.isfile("{0}/{1}".format(curr_dir, 'sync_directory.py'))):
    ...
    fapi._set_session()
    print(fapi.__SESSION.credentials)
