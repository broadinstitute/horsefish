...
"""
# python3 streaming_upload/sync_directory.py --log-file '/Users/msimeon/horsefish/streaming_upload/LOGS/run.201112_M04004_0509_000000000-DB53C.lane.all.log' --project "fc-156ce5ce-ac56-40ee-adcc-71ad6dc9d351" --tar-destination '201112_M04004_0509_000000000-DB53C/runs' --tar-directory '/Users/msimeon/horsefish/streaming_upload/upload/TMP' --include-patterns --exclude-patterns 'Images' --min-tar-size 500 --max-tar-size 10000 --upload-threads 8 --prefix 'run.201112_M04004_0509_000000000-DB53C.lane.all' --auth-token '' --verbose --finish 'streaming_upload/miseq/201112_M04004_0509_000000000-DB53C'
...
def upload_tar_files(tar, args):
    """Uploads any tar files that haven't yet been uploaded"""

    print(sys.stderr, "\n--- Uploading tar files...")
    print(args.tar_destination)
    storage_client = storage.Client()
    bucket = storage_client.bucket(args.project)
    blob = bucket.blob(args.tar_destination + "/" + tar )
    print(blob)
    filename = "{}/{}".format(args.tar_directory, tar)

    res = blob.upload_from_filename(filename)
    print(res.json)
    print(
    "File {} uploaded to {}.".format(
        "tar_filename", "args.tar_destination"
    )
    )
...
def main():
    ...
    for tar in tars_to_upload:
        tar = create_tar_file(tar, log, args)
        log = upload_tar_files(tar, args)
